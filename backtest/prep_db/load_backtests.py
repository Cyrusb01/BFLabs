...
sqlite_db = 'backtests_20190212.sqlite'
conn = sqlite3.connect(sqlite_db)
conn.row_factory = hp.dict_factory
cur = conn.cursor()

# btc_v1p0_20190201
table = "btc_v1p0_20190201"
print(table)
...
df = pd.read_csv('../../../alpha-predator-model-v1p0/test_backtest_v1_BTC_20190201/master.csv')
...
# btc_v1p0_20190201_bayes
table = "btc_v1p0_20190201_bayes"
print(table)
...
df = pd.read_csv('../../../alpha-predator-model-v1p0/test_backtest_v1_BTC_20190201_bayes/master.csv')
df = df[ ['assets','timestamp','price'] ]
df['date'] = (df['timestamp']*1000).astype(int)
df['strategy_return'] = df['assets']/100.0
df['market_return'] = df['price'] * 100 / df.iloc[0]['price']
...
df = pd.read_csv('../../../apm_eth/test_backtest_v1_20181231/master.csv')
df = df[ ['assets','timestamp','price'] ]
df['date'] = (df['timestamp']*1000).astype(int)
df['strategy_return'] = df['assets']/100.0
df['market_return'] = df['price'] * 100 / df.iloc[0]['price']
...
df = pd.read_csv('../../../alpha-predator-model/test_backtest_exp/results.csv')
